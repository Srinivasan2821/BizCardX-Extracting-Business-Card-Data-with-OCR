...
if Menu == "Home":
  ...
  if method == "Preview":
    ...
    # Converting each string in the tuple from 'latin1' to 'utf-8'
    table = []
    for row in raw_table:
        converted_row = tuple(col.decode('latin1') if isinstance(col, bytes) else col for col in row)
        table.append(converted_row)

    # Create a DataFrame from the converted data
    df3 = pd.DataFrame(table, columns=[
